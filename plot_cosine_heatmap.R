...
#' @param plot_axis_labels Plot Axis labels. Default = TRUE.
#' 
...
plot_cosine_heatmap <- function(cos_sim_matrix, col_order = NA, row_order = NA, cluster_rows = TRUE,
                                cluster_cols = FALSE, method = "complete", plot_values = FALSE, plot_axis_labels=TRUE) {
  ...
  if (plot_axis_labels==FALSE){
    plot_final + theme(axis.title.x=element_blank(),
